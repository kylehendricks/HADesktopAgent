...
`config.json` is created by the user before first run, or generated by the Home Manager
module (see below). Key sections:
...
Config location, highest precedence first: `--config <path>`, `$HADESKTOPAGENT_CONFIG`, then
`%LOCALAPPDATA%\HADesktopAgent\config.json` (Windows) / `~/.local/share/HADesktopAgent/config.json`
(Linux). Only that last, user-managed path is created on first run. Environment variables
(`Mqtt__Password`) are added after the file and override it.

`AgentConfigurationSetup` (Core) owns all of this — path resolution, the first-run skeleton,
and the `AddOptions`/validator registrations — so both platform hosts bind the same config
surface. Adding a config section means touching that one file, not both `Program.cs`es.

### Secrets

`Mqtt.PasswordFile` points at a file holding the password, resolved into `Mqtt.Password` at
startup by `MqttSecretFileLoader` (an `IPostConfigureOptions`, so it runs before the
validators). It exists because the Nix module renders config.json into the world-readable
store; the path is safe there, the secret is not.

### Nix

`flake.nix` exposes `packages.default` (`buildDotnetModule`, Linux project only) and
`homeManagerModules.default`. The module's `settings` option is a freeform submodule typed
with `pkgs.formats.json`, rendered to a store path and passed as `--config`. Common keys are
declared so typos fail at build time; everything else passes through. A plaintext
`Mqtt.Password` warns and a store-path `Mqtt.PasswordFile` is an assertion failure.

Note that `buildDotnetModule` builds from the *git-tracked* tree — `git add` a new `.cs` file
before `nix build`, or the build fails on the missing type.
